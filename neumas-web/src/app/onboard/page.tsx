    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-[26px] font-bold tracking-tight text-gray-900">
          Welcome to Neumas Control
        </h1>
        <p className="mt-2 text-[15px] text-gray-500">
          Let's get your workspace configured. This takes about 3 minutes.
        </p>
      </div>
...
function StepOutlets({
  ...
  outlets: Outlet[];
  setOutlets: (v: Outlet[]) => void;
  onNext: () => void;
  onBack: () => void;
}) {
  function addOutlet() {
    setOutlets([...outlets, { name: "", type: "Restaurant" }]);
  }

  function removeOutlet(idx: number) {
    setOutlets(outlets.filter((_, i) => i !== idx));
  }

  function updateOutlet(idx: number, key: keyof Outlet, val: string) {
    setOutlets(outlets.map((o, i) => (i === idx ? { ...o, [key]: val } : o)));
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1.5 text-[14px] text-gray-500">
          Add each location that receives deliveries or uses inventory. You can add more later.
        </p>
      </div>
        ...
        {outlets.map((o, idx) => (
          ...
          >
            ...
            <input
              ...
              className="min-w-0 flex-1 border-none bg-transparent text-[13px] text-gray-900 outline-none placeholder:text-gray-400"
            />
            <select
              value={o.type}
              onChange={(e) => updateOutlet(idx, "type", e.target.value)}
              className="shrink-0 rounded-lg border border-gray-200 bg-gray-50 px-2.5 py-1.5 text-[12px] text-gray-700 outline-none"
            >
              {OUTLET_TYPES.map((t) => (
            ...
            {outlets.length > 1 && (
              <button
                ...
                className="shrink-0 rounded-lg p-1 text-gray-300 hover:text-gray-500"
              >
                <X className="h-4 w-4" />
              </button>
            )}
      ...
      >
        Skip for now — I'll add outlets later
      </button>
    </div>
  );
}
...
function StepUpload({
  ...
  const onFileSelected = useCallback(
    (f: File) => {
      ...
      if (!allowed.includes(f.type)) {
        toast.error("Upload a JPEG, PNG, WebP, or PDF.");
        return;
      }
  ...
  async function runScan() {
    ...
    try {
      ...
      if (!sid) { toast.error("Could not start scan."); setBusy(false); return; }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1.5 text-[14px] text-gray-500">
          Drop any supplier invoice or delivery note. Our AI extracts every line item and
          posts it to inventory automatically.
        </p>
      </div>
      ...
      {done ? (
        <div className="rounded-2xl bg-emerald-50 border border-emerald-100 p-6 text-center">
          ...
          <p className="mt-1 text-[13px] text-emerald-700">
            Your inventory has been updated. Head to the dashboard to see your data.
          </p>
        </div>
      ) : (
        <>
          ...
          <div
            ...
            onClick={() => {
              ...
              inp.onchange = () => { if (inp.files?.[0]) onFileSelected(inp.files[0]); };
              inp.click();
            }}
            className={cn(
      ...
      <div className="flex gap-3">
        ...
        {done ? (
          ...
          <button
            type="button"
            disabled
            className="flex-1 rounded-xl bg-[#0071a3] py-3 text-[14px] font-semibold text-white opacity-40"
          >
      ...
      <button
        ...
        className="w-full text-center text-[12px] text-gray-400 hover:text-gray-600 underline underline-offset-2"
      >
        Skip — I'll upload later from the dashboard
      </button>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6 text-center">
      ...
      <div>
        ...
        <p className="mt-2 text-[15px] text-gray-500">
          Head to your dashboard to see live inventory, upload more documents,
          and let the AI start building your procurement intelligence.
        </p>
      </div>
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={m.label} className="flex items-center gap-3">
              <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-gray-100 font-mono text-[11px] text-gray-500">
                {i + 1}
              </span>
              <div>
      ...
      <button
        type="button"
        onClick={onFinish}
        className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#0071a3] py-4 text-[15px] font-semibold text-white shadow-sm hover:bg-[#005f8a] transition-colors"
      >
        Open my dashboard
        <ArrowRight className="h-4 w-4" />
      </button>
    ...
    <div className="min-h-screen bg-[#f5f5f7]">
      ...
      <div className="mx-auto max-w-lg px-5 py-14">
        ...
        <p className="mb-6 font-mono text-[11px] font-medium tracking-widest text-gray-400 uppercase">
          ...
          {step === 1 ? "Welcome" : step === 2 ? "Outlets" : step === 3 ? "First document" : "Ready"}
        </p>
